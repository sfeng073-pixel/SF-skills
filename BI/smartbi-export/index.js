...
class SmartBIExporter {
    ...
    async _searchAndOpen(page, keyword) {
        ...
        try {
            // 使用evaluate点击搜索结果（兼容首页内嵌和独立页面两种情况）
            ...
            if (!clicked) {
                // 回退到locator方式
                await page.locator(`text=${match.label}`).first().click();
            }
    ...
    async _doSearch(page, keyword) {
        ...
        return { found: true, label: this._selectBestMatch(items, keyword).label };
    }
    ...
    _selectBestMatch(items, keyword) {
        ...
        const scored = items.map(item => {
            let score = 0;
            keyword.split(/[_\s]+/).forEach(part => { if (part.length >= 2 && item.label.includes(part)) score += 20; });
