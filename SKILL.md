...
**解决**：
1. 检查 SmartBI 连接是否正常
2. 确认 config.json 中的报表名称与 SmartBI 中的完全一致
3. 手动到 SmartBI 中搜索报表确认名称

### Q2: 数据解析结果为空或错误
**原因**：表头关键词匹配失败
**解决**：
1. 打开导出的 Excel 文件，确认表头内容
2. 调整 config.json 中 `header_keywords` 的关键词（使用部分关键词即可）
3. 检查 `exclude` 字段是否误排除了有效列

### Q3: 百分比数值显示异常（如显示 7000% 而非 70%）
**原因**：字段类型配置错误
**解决**：
- 如果原始值是 70，使用 `"type": "percent"`
- 如果原始值是 0.7，使用 `"type": "percent_raw"`

### Q4: 排名数据排序不对
**原因**：`sort_by` 字段名与 fields 中的 key 不一致
**解决**：确认 `sort_by` 的值与 `fields` 中定义的字段 key 完全一致

### Q5: 通时通次模块不显示
**原因**：报表 optional=true 且导出失败
**解决**：
1. 检查 config.json 中该报表的 `optional` 设置
2. 确认 SmartBI 中该报表可正常访问
3. 使用 `--files` 参数手动传入 Excel 文件测试

### Q6: 大屏在局域网无法访问
**原因**：防火墙或端口未开放
**解决**：
1. 确认 http-server 启动成功（显示 "Available on:"）
2. 检查电脑防火墙设置，放行 8080 端口
3. 确保手机与电脑在同一局域网
...
2. 或在 config.json 中设置 `refresh_interval: 0` 禁用自动刷新

## 版本更新日志

### v2.1.0 (2024-05-25)
**新增**
- 添加 `.gitignore` 文件，规范版本控制
- 添加 `package.json`，明确依赖管理
- 新增 FAQ 常见问题章节
